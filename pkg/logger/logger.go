...
func init() {
	...
	// 时间编码进行指定格式解析
	encoderConfig.EncodeTime = parseTime(conf.Conf.Layout)

	// 日志输出配置, 借助另外一个库 lumberjack 协助完成日志切割。
	lumberjackLogger := &lumberjack.Logger{
		Filename:   conf.Conf.Filename,   // -- 日志文件名
		MaxSize:    conf.Conf.MaxSize,    // -- 最大日志数 M为单位!!!
		MaxAge:     conf.Conf.MaxAge,     // -- 最大存在天数
		MaxBackups: conf.Conf.MaxBackups, // -- 最大备份数量
		Compress:   false,                // --是否压缩
	}
